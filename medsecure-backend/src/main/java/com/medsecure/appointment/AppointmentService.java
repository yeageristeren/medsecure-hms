...
public class AppointmentService {
    ...
    public AppointmentResponseDto createNewAppointment(CreateAppointmentRequestDto createAppointmentRequestDto) {
        ...
        Long patientId = patientService.getPatientByUsername(patientUsername).getId();
    ...
    public Page<AppointmentResponseDto> getAllAppointmentsOfDoctor(String userName, Pageable pageable) {
        ...
        Doctor doctor = doctorRepository.findById(doctorId).orElseThrow(
                () -> new ResourceNotFoundException("Doctor credentials not found")
        );
    ...
    public Page<AppointmentResponseDto> getAllAppointmentsOfPatient(Pageable pageable) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        AppUser user = userRepository.findByUsername(username)
                .orElseThrow(() -> new EntityNotFoundException("User not found!!"));
        Page<Appointment> appointments = appointmentRepository.getAppointmentsByPatient_Id(user.getId(),pageable);
        return appointments.map(appointment -> {
            AppointmentResponseDto dto = AppointmentResponseDto.builder()
                    ...
                    .patient(modelMapper.map(appointment.getPatient(),PatientResponseDto.class))
                    .doctor(modelMapper.map(appointment.getDoctor(), DoctorResponseDto.class))
                    .build();
            return dto;
        });
    }
