...
public class DoctorService {
    ...
    public Page<DoctorResponseDto> findAvailBySpec(String specialisation, Pageable pageable) {
        ...
        Page<Doctor> doctors = doctorRepository.findBySpecializationContainingIgnoreCaseAndDeleted(specialisation,false,pageable);
        return doctors.map(doctor -> DoctorResponseDto.builder()
                ...
                .specialization(doctor.getSpecialization())
                .build());}
        return doctorRepository.findAll(pageable).map(doctor -> DoctorResponseDto.builder()
                .email(doctor.getEmail())
                .id(doctor.getId())
                .name(doctor.getName())
                .specialization(doctor.getSpecialization())
